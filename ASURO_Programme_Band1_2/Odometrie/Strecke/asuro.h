...
* 2.20     13.07.2006   Robin Gruber     Added Switch calculation without float
*                                        An own timer interrupt function may now be linked in
*                                        SerWrite() now waits exactly till all bytes are transferred before returning
*                                        SerRead() returns an int with the number of bytes received before a timeout occured
*                                        Added nominal, +1% and -1% values for switch calculation
*                                        Some minor improvements for readability
*                                        MSleep() added for waiting whole milliseconds
*
*
...
// nominal values for collision switch calculation

#define SWITCHFACTORINT 131072 // nominal
#define SWITCHFACTORFLOAT 1024.0 // nominal

// values for trim of switch calculation
// try these values if collision switches cause problems
//#define SWITCHFACTORINT 132383 // +1% trim
//#define SWITCHFACTORFLOAT 1034.2 // +1% trim

//#define SWITCHFACTORINT 129761 // -1% trim
//#define SWITCHFACTORFLOAT 1013.8 // -1% trim
...
int SerRead(unsigned char *data, unsigned char length, unsigned int timeout);
...
/* Sleep function waitung milliseconds */
void MSleep(unsigned int ms);

/* Link an own timer routine */
void LinkTimerISR(void (*MyTimerISR)(void));
