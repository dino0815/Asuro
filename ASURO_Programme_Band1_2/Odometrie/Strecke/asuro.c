...
void SerWrite(unsigned char *data,unsigned char length)
{
	...
	while (length > 0) {
		if (UCSRA & (1<<UDRE)) { // wait for empty data register
			UDR = data[i++];
			length --;
		}
	}
	while (!(UCSRA & (1<<TXC))); // wait for end of transmission
	//for (i = 0; i < 0xFE; i++)
...
unsigned char PollSwitch (void)
{
	...
	#ifdef NOFLOAT 
	return  (unsigned char) ((SWITCHFACTORINT/(long)ADC - 128) + 1) >> 1;
	#endif
	#ifndef NOFLOAT
	return  ((unsigned char) ((( SWITCHFACTORFLOAT/(float)ADC - 1.0)) * 64.0 + 0.5));
	#endif
}
